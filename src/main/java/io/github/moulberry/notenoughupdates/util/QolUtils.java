...
public class QolUtils {
	...
	private static void drawLine(WorldRenderer bufferBuilder, BlockPos pos, BlockPos nextPos) {
		bufferBuilder.pos(pos.getX() + 0.5f, pos.getY() + 0.5f, pos.getZ() + 0.5f).color(255,0,0,255).endVertex();
		bufferBuilder.pos(nextPos.getX() + 0.5f, nextPos.getY() + 0.5f, nextPos.getZ() + 0.5f).color(255,0,0,255).endVertex();
	}
	...
	public static void drawLine(RenderWorldLastEvent event, Vec3 from, Vec3 to, float lineWidth, Color color) {
		...
		drawLine(bufferBuilder, from, to, color);
